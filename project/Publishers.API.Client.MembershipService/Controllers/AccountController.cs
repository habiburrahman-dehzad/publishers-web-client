...
namespace Publishers.API.Client.MembershipService.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            switch (result)
            {
                case Identity.SignInStatus.Success:
                    return RedirectToLocal(returnUrl);
                case Identity.SignInStatus.LockedOut:
                    return View("Lockout");
                case Identity.SignInStatus.RequiresTwoFactorAuthentication:
                    return RedirectToAction("SendCode", new { ReturnUrl = returnUrl });
                case Identity.SignInStatus.Failure:
                default:
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                {
                    ViewBag.Message = "Could Not connect to mongod server and the to the replica set. Make sure all instances of the server are running.";
                    return View("Error");
                }
                catch (Exception)
                {
                    ViewBag.Message = "An Unknow error occured while creating the new user. Either the server is not running or it is not configured properly"
                        + " or you don't have the credentials to connect to server.";
                    return View("Error");
                }
            }
